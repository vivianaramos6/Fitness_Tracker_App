...
def display_app_page():
    ...
    with tabs[3]:
        with tabs[3]:
            st.title("Gen AI Fitness Advice")
            col1, col2 = st.columns([2, 4])  

            with col1:
                st.header("Fitness Tips")
                st.write("Ask for personalized advice in the chat")

            with col2:
                st.header("Get Advice")
        
                user_input = st.text_input("What fitness advice would you like?", key="user_input")
        
                if st.button("Get Advice") and user_input:
                    try:
                        # Get advice from data_fetcher
                        advice_data = get_genai_advice(user_id=1, user_input=user_input)
                        ...
                        st.error(f"Couldn't generate advice. Please try again. Error: {str(e)}")
